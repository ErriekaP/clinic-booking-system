...
const CancelQueueDialog = ({ queueId }: { queueId: string }) => {
  const router = useRouter();

  const handleYesClick = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/queue/cancel/${queueId}`,
        {
          method: "GET",
        }
      );
    ...
    <Dialog.Root>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="DialogContent">
          <Dialog.Title className="DialogTitle">Cancel Queue</Dialog.Title>
